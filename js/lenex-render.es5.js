...
function toggleClass(el, toToggle) {
    if (isClassSet(el, toToggle)) {
        removeClass(el, toToggle);
    } else {
        addClass(el, toToggle);
    }
}
...
Lenex.prototype.customise = function () {
    ...
    this.numColumns = 3;
    if (this.customisation.options.showClub) {
        this.numColumns++;
    }
    if (this.customisation.options.showYoB) {
        this.numColumns++;
    }
    if (this.customisation.options.showAge) {
        this.numColumns++;
    }
};
...
Lenex.prototype.setMode = function () {
    if (this.containerEl.clientWidth > 768 && this.customisation.options.displayMode == 0) {
        ...
        if (this.mode != 0) {
            ...
            this.mode = 0;
            this.meets.forEach(function (meet) {
                meet.mode = 0;
            });
        }
    } else {
...
Lenex.prototype.handleEvent = function (event) {
    switch (event.type) {
        case 'resize':
            if (this.backgroundImageEl != undefined) {
...
Lenex.prototype.render = function () {
    ...
    var lenex = this;
    this.meets.forEach(function (meet) {
        containerEl.appendChild(meet.render(lenex));
    });
    return containerEl;
};
...
function createWhatToShowControl(meetEl) {
    ...
    var selectEl = createDropDown('what-to-show', [{ value: 'show-auto', text: 'Results or start lists' }, { value: 'show-startlists', text: 'Start lists' }, { value: 'show-results', text: 'Results' }, { value: 'show-results-by-heat', text: 'Results by heat' }]);
...
function createSplitsModeControl(meetEl) {
    ...
    var selectEl = createDropDown('splits-mode', [{ value: 'splits-off', text: 'Hide splits (click times to toggle)' }, { value: 'splits-time', text: 'Show all splits as times' }, { value: 'splits-interval', text: 'Show all splits as intervals' }]);
    selectEl.onchange = function () {
        setSelectorClasses(selectEl, meetEl);
    };
    setSelectorClasses(selectEl, meetEl);
...
Meet.prototype.render = function (lenex) {
    ...
    if (lenex.customisation.options.displayMode == 0) {
        ...
        this.sessions.forEach(function (session) {
            indexEl.appendChild(session.render(eventInfoContainerEl, this, lenex));
        }, this);
    } else {
        // displayMode 1
...
Session.prototype.render = function (separateEventInfoEl, meet, lenex) {
    ...
    this.events.forEach(function (event) {
        ...
        titleEl.onclick = function () {
            eventClicked(titleEl, event, meet, lenex);
        };
...
Event.prototype.renderStartLists = function (lenex, meet) {
    ...
    this.heats.forEach(function (heat) {
        el.appendChild(heat.render(lenex, meet));
    });
    return el;
};
Event.prototype.renderResults = function (lenex, meet) {
    ...
    this.agegroups.forEach(function (agegroup) {
        el.appendChild(agegroup.render(lenex, meet));
    });
    return el;
};
Event.prototype.renderResultsByHeat = function (lenex, meet) {
    ...
    this.heats.forEach(function (heat) {
        el.appendChild(heat.renderResults(lenex, meet));
    });
    return el;
};
...
function addAthleteColumnHeadings(rowEl, lenex) {
    ...
    if (lenex.customisation.options.showClub) {
        addColumnHeading(rowEl, 'club', 'Club');
    }
    if (lenex.customisation.options.showYoB) {
        addColumnHeading(rowEl, 'yob', 'YoB');
    }
    if (lenex.customisation.options.showAge) {
        addColumnHeading(rowEl, 'age', 'Age');
    }
}
...
Heat.prototype.renderResults = function (lenex, meet) {
    ...
    heatResults.forEach(function (result) {
        if (result.seconds != previousRaceTime) {
            placeToDisplay = row;
        }
...
Result.prototype.renderToTableWithSplits = function (tableEl, placeToDisplay, row, lenex, meet) {
    ...
    if (isOdd) {
        addClass(resultRowEl, 'odd');
    }
    tableEl.appendChild(resultRowEl);
    if (this.splits.length > 0) {
        var splitsRowEl = this.renderSplits(lenex);
        if (isOdd) {
            addClass(splitsRowEl, 'odd');
        }
        resultRowEl.timeEl.onclick = function () {
            toggleClass(splitsRowEl, 'show');
        };
        splitsRowEl.onclick = function () {
            toggleClass(splitsRowEl, 'interval');
        };
